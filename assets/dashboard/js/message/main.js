$(document).ready(function () {
	...
	function getUserList() {
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			$.get('Message/ownerDetails', function (data) {
				...
				dob = jsonData[0]['dob'];
				phone = jsonData[0]['instagram'];
				addr = jsonData[0]['geoname'];
				bio = jsonData[0]['about'];
	...
	function setUserDetails(data) {
		...
		var avtar = `assets/dashboard/img/profile/${data[0]['profile_picture']}`;
		...
		(data[0]['about'].length > 1) ? $('#details_of_bio').html(data[0]['about']) : $('#details_of_bio').html("--Not Given--");
		$('#details_of_email').html(`<span><i class="fas fa-envelope text-dark pr-2"></i></span>${data[0]['email']}`);
		...
		(data[0]['instagram'].length > 1) ?
			$('#details_of_mobile').html(`<span><i class="fas fa-mobile-alt text-dark pr-2"></i></span>${data[0]['instagram']}`) :
			$('#details_of_mobile').html(`<span><i class="fas fa-mobile-alt text-dark pr-2"></i></span>--Not Given--`);

		(data[0]['geoname'].length > 1) ?
			$('#details_of_location').html(`<span><i class="fas fa-map-marker-alt text-dark pr-2"></i></span>${data[0]['geoname']}`) :
	...
	function sendUserUniqIDForMsg(uniq_id, bg_image) {
		$.post('Message/getmessage', { data: uniq_id, image: bg_image }, function (data) {
	...
	$('#send_message').click(function (e) {
		...
		$.post('Message/sendMessage', { data: jsonData }, function (data) {
			$('#messageText').val('');
		})
	})
