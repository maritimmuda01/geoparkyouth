<script type="text/javascript">
	...
	//Country
	function tampilCountry() {
		$.get('<?php echo base_url('Country/tampil'); ?>', function(data) {
			MyTable.fnDestroy();
			$('#data-country').html(data);
			refresh();
		});
	}

	var id_country;
	$(document).on("click", ".konfirmasiHapus-country", function() {
		id_country = $(this).attr("data-id");
	})
	$(document).on("click", ".hapus-dataCountry", function() {
		var id = id_country;
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Country/delete'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#konfirmasiHapus').modal('hide');
	...
	$(document).on("click", ".update-dataCountry", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Country/update'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#tempat-modal').html(data);
	...
	$(document).on("click", ".detail-dataCountry", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Country/detail'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
			$('#tempat-modal').html(data);
			...
			$('#detail-country').modal('show');
		})
	})

	$('#form-tambah-country').submit(function(e) {
		var data = $(this).serialize();

		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('Country/prosesTambah'); ?>',
			data: data
		})
		.done(function(data) {
			var out = jQuery.parseJSON(data);
			...
			if (out.status == 'form') {
				...
				document.getElementById("form-tambah-country").reset();
				$('#tambah-country').modal('hide');
				$('.msg').html(out.msg);
				effect_msg();
			}
		})
	...
	$(document).on('submit', '#form-update-country', function(e){
		...
		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('Country/prosesUpdate'); ?>',
			data: data
		})
		.done(function(data) {
			var out = jQuery.parseJSON(data);
			...
			if (out.status == 'form') {
				...
				document.getElementById("form-update-country").reset();
	...
	$('#tambah-country').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})

	$('#update-country').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})
