...
class M_user extends CI_Model {
	...
	public function select_all(){
		...
		$data = $this->db->query("SELECT user.id as id, user.name as name, user.email as email, user.profile_picture as profile_picture, user.gender as gender, user.dob as dob, user.country as country, user.position as position, user.company as company, user.twitter as twitter, user.instagram as instagram, user.linkedin as linkedin, user.role_id as role_id, country.iso as iso, country.nicename as nicename, geoname.iso as geoname_iso, geoname.name as geoname_name, user.geoname as geoname FROM user,country,geoname WHERE user.country = country.iso AND is_active=1 AND geoname.iso = user.geoname AND user.id != 1 ORDER BY name ");
