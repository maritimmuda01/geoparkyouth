...
class M_pages extends CI_Model {
	...
	public function select_single($id) {
		...
		$data = $this->db->query("SELECT pages.id as id, pages.title as title, pages.slug as slug, pages.content as content, pages.parent_id as parent_id, page_parent.id as pages_parent_id, page_parent.name as page_parent_name, pages.image as image FROM pages, page_parent WHERE pages.parent_id = page_parent.id AND pages.id = {$id}");
