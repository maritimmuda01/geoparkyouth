...
<?php if ($profile->role_id == 1) {
  $url = 'jobvacancy/showAllPendingJobvacancy/';
} else if ($profile->role_id == 3) {
  $url = 'jobvacancy/showAllPendingJobvacancyByCountry/' . $profile->id_country;
} ?>

<script>
  $(function() {
    showAllPendingJobvacancy();
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
              type: 'get',
              async: false,
              url: '<?php echo base_url() ?>jobvacancy/publishJobvacancy',
              data: {
                id: id
              },
              dataType: 'json',
    ...
    //function
    function showAllPendingJobvacancy() {
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i = 0; i < data.length; i++) {
            html += '<tr>' +
              ...
              '<button class="btn btn-info item-publish mr-2" data="' + data[i].id_job + '">Publish</button>' +
