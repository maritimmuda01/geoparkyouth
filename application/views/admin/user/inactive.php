      ...
      <div class="modal-body">
        <form id="myForm" action="#" method="post" class="needs-validation" novalidate="" onsubmit={this.saveAndContinue}>
          ...
          <div class="form-group has-error">
            <label>Name</label>
            <input type="text" class="form-control" name="name" tabindex="1" required="" readonly>
          </div>
...
<?php if ($profile->role_id == 1) {
  $url = 'user/showAllInactiveUser/';
} else if ($profile->role_id == 3) {
  $url = 'user/showAllInactiveUserByCountry/' . $profile->id_country;
} ?>
...
<script>
  $(function() {
    ...
    $('#btnSave').click(function() {
      ...
      var result = '';
      if (userName.val() == '') {
        userName.parent().addClass('was-validated');
      } else {
        userName.parent().parent().removeClass('was-validated');
        result += 1;
      }
    ...
    //activate
    $('#show_data').on('click', '.item-activate', function() {
      var id = $(this).attr('data');
      swal({
          title: 'Activate User?',
          icon: 'info',
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
              ...
              method: 'get',
              async: false,
              url: '<?php echo base_url() ?>user/activateUser',
              data: {
                id: id
              },
              dataType: 'json',
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i = 0; i < data.length; i++) {
            ...
            html += '<tr>' +
              '<td>' + (i + 1) + '</td>' +
              '<td class=""><img alt="image" src="<?php echo base_url(); ?>images/profile/' + data[i].profile_picture + '" class="rounded-circle mr-4" width="30" height="30">' + data[i].name + '</td>' +
              ...
              '<button class="btn btn-primary item-activate" data="' + data[i].id_user + '">Activate</button>' +
              '<button class="btn btn-danger item-delete" style="margin-left:10px" data="' + data[i].id_user + '"><i class="fa fa-trash" aria-hidden="true"></i></button>' +
              '</td>' +
              '</tr>';
          }
          MyTable.fnDestroy();
