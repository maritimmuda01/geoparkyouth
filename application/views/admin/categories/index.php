    ...
    $('#show_data').on('click', '.item-edit', function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          $('input[name=name]').val(data.category_name);
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i = 0; i < data.length; i++) {
            html += '<tr>' +
              '<td class="">' + (i + 1) + '</td>' +
              '<td class="">' + data[i].id_category + '</td>' +
              '<td class="">' + data[i].category_name + '</td>' +
              '<td class="text-center">' +
              '<button class="btn btn-warning item-edit" data="' + data[i].id_category + '"><i class="far fa-edit" aria-hidden="true"></i></button>' +
              '<button class="btn btn-danger item-delete" style="margin-left:10px" data="' + data[i].id_category + '"><i class="fa fa-trash" aria-hidden="true"></i></button>' +
              '</td>' +
              '</tr>';
          }
          MyTable.fnDestroy();
